public class Main {
    public static void main(String[] args) {
        ...
        printMonth(12);
        checkYearHighQuality(year);
        determiningVersionPhone(clientOS, clientDeviceYear);
        deliveryTime = deliveryTime(deliveryDistance);
...
// Если год не високосный, то, соответственно: “*номер года —* не **високосный год”.
    public static void checkYearHighQuality(int year) {
        if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) {//{System.out.println("«"+ year +" год не является високосным»");}
...
//    от этого зависит, подойдет ли приложение длятеле фона или оно просто не установится.
...
//    Ваша задача: написать программу, которая выдает соответствующее сообщение клиенту при наличии двух
//    условий — операционной системы телефона (iOS или Android) и года производства.
...
//    (обычную или lite) и для какой ОС (Android или iOS) нужно установить пользователю.
    public static void determiningVersionPhone(byte clientOS, int clientDeviceYear) {
        String liteV = "";
        if (clientDeviceYear < 2015) {
            liteV = " облегченную";
        }
        if (clientOS == 0) {
            System.out.println("Установите" + liteV + " версию приложения для iOS по ссылке");
        } else {
            System.out.println("Установите" + liteV + " версию приложения для Android по ссылке");
        }
    }
...
//    на дом с четкой датой доставки. Чтобы известить клиента о том, когда будет доставлена его карта, нам нужно
...
//    Наша задача — доработать код, а именно написать метод, который на вход принимает дистанцию и возвращает итоговое количество дней доставки.
    public static int deliveryTime(int deliveryDistance) {
        ...
        if (deliveryDistance > 20) {
            zone = deliveryDistance / 40;
        }
        return deliveryTime + zone;
    }

    public static void printMonth(int monthNumber) {
        switch ((monthNumber / 3) + 1) {
            case 1:
            case 5:
